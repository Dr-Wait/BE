...
//                .profileImageUrl("기본 이미지 URL")
  ...
  public LoginResponseDto hospitalLogin(LoginRequestDto request) {
    ...
    // TODO: Role 를 DB에 저장할지 고민
    String accessToken = jwtTokenProvider.generateAccessToken(hospital.getId().toString(),
        Role.ADMIN.name());
    String refreshToken = jwtTokenProvider.generateRefreshToken(hospital.getId().toString(),
        Role.ADMIN.name());
